...
class RequestCorporateAccounts extends Model
{
    ...
    public function rules()
    {
        return [
            [['name', 'email'], 'required'],
            ['email', 'email'],
            [['cell_phone', 'from', 'to', 'dep_date', 'arr_date', 'fare', 'message'], 'safe'],
            [['name', 'cell_phone', 'email', 'from', 'to', 'fare', 'message'], 'string'],
            [['message'], 'string', 'max' => 500],
            [['name'], 'string', 'max' => 100],
            [['email'], 'string', 'max' => 100],
            [['cell_phone'], 'string', 'max' => 100],
        ];
    }
    ...
    public function sendEmailRequestCorporateAccount($email, $model)
    {
        return Yii::$app->mailer
            ...
            ->setTo($email)
            ->setSubject('FlyFirst - New Corporate Account Message')
            ->send();
    }
}