    ...
    public function actionCreate()
    {
        ...
        if($city_model->load(Yii::$app->request->post())){
            ...
            if($isValid && $imgValidation)
            {
                ...
                Yii::$app->session->addFlash('info', 'City '.$city_model->name.' add. <a href="'. Url::to(['/city'], true) .'/'.$city_model->alias .'" >View</a>');
    ...
    public function actionEdit()
    {
        ...
        if($city_model->load(Yii::$app->request->post()))
        {
            ...
            if($isValid && $imgValidation)
            {
                ...
                Yii::$app->session->addFlash('info', 'City '.$city_model->name.' update. <a href="'. Url::to(['/city'], true) .'/'.$city_model->alias .'" >View</a>');
                $this->redirect(['adminarea/city']);
            }
            else {
                $err = array();
