jQuery(document).ready(function ($) {
    var isFront = isLandingPage = isAirlinePage = isBlogPage = isBlogArticlePage = isTravelTipsPage =
        isTravelTipsAttactionsPage = isAboutPage = isHotelPage = isCorporateAccountsPage = isFlightRequestMax = false;
    ...
    if (isFlightRequestMax) {
        ...
        $(function () {
            ...
            /* animate */
            var FormAnimateFunction = function () {
                var isAnimateActive = false,
                    input = formBlockWrapper.find("input");
                input.click(function () {
                    if (isAnimateActive == false) {
                        isAnimateActive = true;
                        topContentWrapper.addClass("active");
                        formBlockWrapper.addClass("active");
                        contactClockWrapper.addClass("visible");
                    }
                });
            };
            //FormAnimateFunction();
            ...
            $(function () {
                ...
                formBlockWrapper.find("[data-destination-id='1']").find(".date-multi-city").addClass("datepicker");
                InitCustomUI();
                ...
                var AddFlight = function () {
                    addDestination.click(function () {
                        ...
                        if (currentCountRowDestination < totalRowDestination) {
                            destinationRowClone.attr("data-destination-id", currentCountRowDestination + 1).appendTo(destinationBlockWrapper).find(".date-multi-city").addClass("datepicker").attr("id", "dep-date-multi-city-" + currentCountRowDestination + 1);
                            RemoveFlight();
                            IsAddFlightButton();
                            InitCustomUI();
                            AutocompleteAirport();
            ...
            var AutocompleteAirport = function () {
                ...
                autocomplete.autocomplete({
                    source: function (request, response) {
                        $.ajax({
                            url: document.location.origin + "/wp-content/themes/bookmefirstclass/searchairport.php",
                            dataType: "json",
                            type: "POST",
                            data: {
    ...
    /*parallax*/
    if (isFront || isLandingPage || isBlogPage || isBlogArticlePage || isTravelTipsAttactionsPage) {
